...
export const ClosetApp = () => {
  ...
  const superiorItems = clothingItems
    .filter(item => item.category === 'superior')
    .sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime())
    .map(item => item.url);

  const inferiorItems = clothingItems
    ...
    <div className="min-h-screen bg-gradient-to-br from-background to-muted/30 p-4">
      ...
      <main className="max-w-md mx-auto space-y-8">
        ...
        <div className="animate-fade-in" style={{ animationDelay: '0.1s' }}>
          <Gallery
            title="Superior"
            images={superiorItems}
            onImageSelect={(index) => handleImageSelect('superior', index)}
          />
        </div>
        ...
        <div className="animate-fade-in" style={{ animationDelay: '0.2s' }}>
          <Gallery
            title="Inferior"
            images={inferiorItems}
            onImageSelect={(index) => handleImageSelect('inferior', index)}
          />
        </div>
      ...
      <Button
        className="closet-btn animate-scale-in"
